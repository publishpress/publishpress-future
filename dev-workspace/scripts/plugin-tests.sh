...
scripts_path="${PROJECT_PATH}/dev-workspace/docker/scripts"
dev_workspace_path="${PROJECT_PATH}/dev-workspace"
...
#######################################
get_plugin_name() {
    "${scripts_path}/parse-json.sh" "${PROJECT_PATH}/composer.json" name | awk -F/ '{print $NF}'
}
...
codeception_envs_path="${PROJECT_PATH}/tests/codeception/_envs"
dir_in_test_container="/var/www/html"
cols=$(tput cols)
...
#######################################
start_services() {
    docker-compose -f $PROJECT_PATH/tests/docker/docker-compose-tests-$php_version.yml -p "${container_name}" up -d
}
...
#######################################
stop_services() {
    docker-compose -f "${PROJECT_PATH}/tests/docker/docker-compose-tests-${php_version}.yml" -p "${container_name}" down
}
...
#######################################
get_mount_path() {
    mount_path=$(docker inspect -f '{{range.Mounts}}{{.Source}}{{end}}' "${wordpress_container_name}")    # Remove from mount_path, all the path before /tests/codeception
    mount_path=${mount_path#*"/tests/docker/"}
    mount_path="${PROJECT_PATH}/tests/docker/${mount_path}"
...
# Globals:
#   wordpress_container_name
#   dir_in_test_container
#######################################
fix_volume_permissions() {
    docker exec "${wordpress_container_name}" find "${dir_in_test_container}" -type d -exec chmod 777 {} \;
    docker exec "${wordpress_container_name}" find "${dir_in_test_container}" -type f -exec chmod 666 {} \;
}
...
create_env_file() {
    ...
    template_file_path="${PROJECT_PATH}/tests/env.acceptance.template.yml"
    ...
    test_site_db_host="host.docker.internal"
    test_site_db_port="${db_container_port}"
    test_site_db_name="wordpress"
    test_site_db_user="root"
    test_site_db_password="wordpress"
    test_site_wp_url="http:\/\/host.docker.internal:${wordpress_container_port}"
    test_site_wp_domain="host.docker.internal:${wordpress_container_port}"
    ...
    replace_placeholder "%TEST_SITE_DB_HOST%" "${test_site_db_host}" "${env_file_path}"
    replace_placeholder "%TEST_SITE_DB_PORT%" "${test_site_db_port}" "${env_file_path}"
    replace_placeholder "%TEST_SITE_DB_NAME%" "${test_site_db_name}" "${env_file_path}"
    replace_placeholder "%TEST_SITE_DB_USER%" "${test_site_db_user}" "${env_file_path}"
    replace_placeholder "%TEST_SITE_DB_PASSWORD%" "${test_site_db_password}" "${env_file_path}"
    replace_placeholder "%TEST_SITE_WP_URL%" "${test_site_wp_url}" "${env_file_path}"
    replace_placeholder "%TEST_SITE_WP_DOMAIN%" "${test_site_wp_domain}" "${env_file_path}"
    replace_placeholder "%TEST_SITE_NAME%" "${test_site_name}" "${env_file_path}"
    replace_placeholder "%TEST_SITE_ADMIN_USERNAME%" "${test_site_admin_username}" "${env_file_path}"
    replace_placeholder "%TEST_SITE_ADMIN_PASSWORD%" "${test_site_admin_password}" "${env_file_path}"

    wp_root_folder=$(echo "${mount_path}" | sed "s/\//:::/g")
    replace_placeholder "%WP_ROOT_FOLDER%" "${wp_root_folder}" "${env_file_path}"
    replace_placeholder ":::" "/" "${env_file_path}"
}

replace_placeholder() {
  # Set the placeholder and replacement values
  placeholder="$1"
  replacement="$2"
  file_path="$3"
  ...
  file=${file//$placeholder/$replacement}
...
#######################################
clean_volumes() {
    rm -rf "${PROJECT_PATH}/tests/docker/volumes/php*"
}
...
run_bootstrap() {
    mount_path=$(get_mount_path)
    $dev_workspace_path/docker/scripts/test-bootstrap.sh "${mount_path}"
}
...
#######################################
run_builder() {
    HIDE_HEADER=1 yarn run build:dir
}
...
#######################################
get_php_versions() {
    find $PROJECT_PATH/tests/docker/ -type f -name 'docker-compose-tests-php*\.yml' | sed 's/tests\/docker\/docker-compose-tests-//g' | sed 's/\.yml//g' | sed 's/\/\//\//g'
}
...
start_chromedriver() {
    ...
    # Docker based chromedriver
    # Check: https://github.com/SeleniumHQ/docker-selenium
    docker run -d -p 4444:4444 --shm-size="2g" selenium/standalone-chrome:4.8.3-20230404
}
...
#######################################
stop_chromedriver() {
    #  Only needed for local chromedriver
...
"brun")
    ...
    "$PROJECT_PATH"/vendor/bin/codecept clean

    echo_step "Running codeception tests: ${*:3}, env: ${php_version}"

    echo ""
    "$PROJECT_PATH"/vendor/bin/codecept run "${*:3}" --env "${php_version}"
